...
export function defaultDispositionForLine(
  line: Pick<Da2062InLine, "nsn" | "serial" | "lin">,
...
export function planDa2062Confirm(input: {
  ...
  accepted: Da2062InLine[];
  skipped: Da2062InLine[];
  flagged: Da2062InLine[];
  conflictDiscrepancies: SourceConflict[];
  flaggedDiscrepancies: SourceConflict[];
  willWrite: boolean;
} {
  const accepted: Da2062InLine[] = [];
  const skipped: Da2062InLine[] = [];
  const flagged: Da2062InLine[] = [];
